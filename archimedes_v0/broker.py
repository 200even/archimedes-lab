...
class BrokerError(RuntimeError):
    """Raised when an action would violate the frozen V0 execution contract."""
...
class ExperimentBroker:
    ...
    def __init__(
        ...
        *,
        ...
        self._closed_result: D4Result | NoConceptResult | None = None
    ...
    def remaining_budget(self) -> dict[str, int]:
        return {
            ...
            "total": BROKER_BUDGET_TOTAL - self._a_count - self._b_cal_count - self._transfer_count,
        }
    ...
    def run_visible_experiment(self, value: ExperimentAST | dict[str, Any]) -> Observation:
        ...
        if self._phase == Phase.A_DISCOVERY:
            ...
            if self._a_count >= BUDGET_A_DISCOVERY:
                raise BrokerError("A discovery budget exhausted; freeze a theory")
            ...
            if self._b_cal_count >= BUDGET_B_CALIBRATION:
                raise BrokerError("B calibration budget exhausted; submit B theory")
        else:
    ...
    def declare_no_concept(self) -> NoConceptResult:
        """Irreversibly abstain after A when no explanatory concept is justified.

        This condition-blind path is required for the preregistered Null-World
        false-positive test. Choosing it on a causal world is simply a D4 failure.
        Unused B budget is forfeited and the run cannot be reopened.
        """
        if self._phase != Phase.A_DISCOVERY or self._a_count != BUDGET_A_DISCOVERY:
