...
FREEZE_PATH = ROOT / "V1_CRITIC_LIVE_EXECUTION_FREEZE.json"
...
HASH_PATHS = (
    ...
    "scripts/run_v1_critic_qualification_live.py",
    ".github/workflows/v1_critic_qualification_live.yml",
    ".github/workflows/v1_determinism.yml",
    "V1_CRITIC_QUALIFICATION_FIXTURES.json",
    "V1_CRITIC_QUALIFICATION_PACKAGE_SPEC.md",
    "V1_CRITIC_QUALIFICATION_RESULT_TEMPLATE.json",
    "V1_CRITIC_LIVE_EXECUTION_FREEZE.json",
...
def _file_manifest() -> dict[str, Any]:
    ...
    for relative in paths:
        path = ROOT / relative
        if not path.is_file():
            raise RuntimeError(f"required freeze path missing: {relative}")
        rows[relative] = {
...
def _load_frozen_execution() -> dict[str, Any]:
    ...
    if raw.get("status") != "FROZEN_FOR_AUTHORIZED_THREE_CALL_CRITIC_SAFEGUARD":
        raise RuntimeError("live Critic execution freeze is absent or not in frozen status")
    if raw.get("authorized_live_critic_calls") != 3:
        raise RuntimeError("live Critic freeze must authorize exactly three calls")
    if raw.get("benchmark_exposure_authorized") is not False:
        raise RuntimeError("live Critic freeze must explicitly prohibit benchmark exposure")
    ...
    if not isinstance(ci, dict) or ci.get("conclusion") != "success" or not isinstance(ci.get("run_id"), int):
        raise RuntimeError("live Critic freeze must identify a successful pre-call V1 determinism run")
    return raw
...
def _result_document(execution, records, execution_commit: str) -> dict[str, Any]:
    ...
    return {
        "package_version": "v1-critic-qualification-1",
        "status": "LIVE_RESULT",
        ...
        "notes": "Generated under the frozen V1 Critic evidence-package contract. No automatic retries or replacement calls were performed.",
    }
...
def main() -> int:
    ...
    # Missing credentials means zero provider requests. It is recorded as an
    # infrastructure abort, never converted into a Critic miss and never retried.
    ...
    result_path = out_dir / "V1_CRITIC_QUALIFICATION_RESULT.json"
    usage_path = out_dir / "V1_CRITIC_QUALIFICATION_USAGE.json"
    ...
    final_manifest = {
        "manifest_version": "v1-final-preexposure-1",
        "status": "GENERATED_AFTER_AUTHORIZED_CRITIC_SAFEGUARD",
        "execution_commit_sha": execution_commit,
        "terminal_execution_class": execution.terminal_execution_class,
    ...
    _write_json(out_dir / "V1_FINAL_PREEXPOSURE_MANIFEST.json", final_manifest)
    checkpoint = _checkpoint(execution, usage_sink.records, execution_commit, result_sha, usage_sha, freeze)
    (out_dir / "REFEREE_CHECKPOINT_V1_FINAL_PREEXPOSURE.md").write_text(checkpoint, encoding="utf-8", newline="\n")

    # Scientific pass/fail is a completed execution and therefore exits 0. An
    # infrastructure abort exits nonzero after all evidence files are emitted.
