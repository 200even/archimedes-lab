...
def _ground_truth_theory(public, hidden, *, include_b=False) -> TheoryAST:
    # Tests only: translate hidden generator programs into the public Theory AST so
    # broker invariants can be tested with a known valid theory. Production agents
    # never receive hidden specs or this translation.
...
def _fill_a(broker, public):
    # 4 actions/entity gives 64 interventions and sufficient coverage.
...
def test_no_concept_abstention_is_condition_blind_and_irreversible():
    for null_world in (False, True):
        ...
        assert result.a_interventions == 64
...
def test_a_program_depth_limit_is_enforced():
    public, hidden, _ = generate_world(808)
    broker = ExperimentBroker.from_generated_world(public, hidden)
    _fill_a(broker, public)
    expr = AddModExpr(left=VarExpr(name="qhat"), right=VarExpr(name="x"))
    # Additional nested operators exceed the frozen maximum expression depth of six.
    for _ in range(6):
        expr = AddModExpr(left=expr, right=ConstExpr(value=1))
    theory = TheoryAST(
        theory_id="T-main",
        latent_variables=[LatentVariable(name="qhat", assignments=dict(hidden.latent_q_by_entity))],
        programs=[ProgramSpec(paradigm="A", expression=expr)],
    )
    with pytest.raises(BrokerError, match="depth limit"):
